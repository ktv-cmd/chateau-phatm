-- Simplified Schema: Just clients and one hardcoded admin
-- Admin email: admin@chateau-demo.com

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Users table (extends Supabase auth.users)
-- No role column needed - we'll check email directly
...
-- Helper function to check if user is admin (hardcoded email)
...
AS $$
  SELECT EXISTS (
    ...
    FROM auth.users
    WHERE id = auth.uid()
      AND email = 'admin@chateau-demo.com'
  );
$$;
...
DROP POLICY IF EXISTS "Admin can view all profiles" ON public.customer_profiles;
